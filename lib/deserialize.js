const { factory, SHELVES } = require('./factory');
...
function deserialize(input) {
  ...
  for (let i = 0; i < refs.length; i += 1) {
    const ref = refs[i];
    const deserializer = factory.get(ref.className, SHELVES.deserializers);
    const obj = deserializer ? deserializer(ref.value) : factory.getInstance(ref.className, undefined) || {};
    objs.push(obj);
  }
